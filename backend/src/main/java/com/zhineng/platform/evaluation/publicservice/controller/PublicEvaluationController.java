...
@RestController @RequestMapping("/api/public-evaluations") public class PublicEvaluationController {
 ...
 @GetMapping Map<String,Object>list(@RequestParam(required=false)Long orgUnitId,@RequestParam(required=false)String source,@RequestParam(required=false)String status,@RequestParam(required=false)Integer minScore,@RequestParam(required=false)Integer maxScore,@RequestParam(required=false)Boolean anonymous,@RequestParam(required=false)String fromDate,@RequestParam(required=false)String toDate,@RequestParam(defaultValue="1")Integer page,@RequestParam(defaultValue="10")Integer size){return service.page(orgUnitId,source,status,minScore,maxScore,anonymous,fromDate,toDate,page,size);}
